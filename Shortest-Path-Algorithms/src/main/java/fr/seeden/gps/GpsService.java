...
public class GpsService {
    ...
    // The maximum number of thread to be used by the process. They are distributed for each ExecutorService below.
    private static int threadNb = (int) (MAX_THREADS*0.75);
    ...
    public static CompletableFuture<List<Node>> findPathBetweenNodes(Node from, Node to, Algorithm.VisitDebugCallback visitDebugCallback, Consumer<List<Node>> acceptCallback){
        ...
        CompletableFuture<List<Node>> completableFuture = CompletableFuture.supplyAsync(() -> algorithm.process(from, to, debugCallback), SUPPLY_SERVICE);
     ...
     *  {@code threadNb}
     * @param threadNb The maximum number of thread.
     * Could not be less than 2 threads (1 per service), and more than three-quarter of the maximum CPU threads.
     ...
     */
    public static void setThreadNb(int threadNb) {
        GpsService.threadNb = Math.clamp(threadNb, 2, (int) (MAX_THREADS*0.75));
        shutdownServices();
        initExecutorServices();
    }
    ...
    private static void initExecutorServices(){
        int resultThreadNb = Math.max((int)(GpsService.threadNb*0.2), 1);
        int supplyThreadNb = Math.max(GpsService.threadNb-resultThreadNb, 1);
        SUPPLY_SERVICE = Executors.newFixedThreadPool(supplyThreadNb);
        RESULT_SERVICE = Executors.newFixedThreadPool(resultThreadNb);
    }
